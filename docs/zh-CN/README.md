...
Squirrel 采用有代理（Agent）架构设计，支持一个 API Server 管理多台服务器上的 Agent：

```mermaid
C4Container
    title Squirrel 系统架构图

    Person(user, "运维人员", "管理系统")

    Container(spa, "Frontend", "Vue3/TypeScript", "Web 前端界面")
    Container(api, "API Server", "Go/Gin", "控制台服务端，提供 API 接口")
    ContainerDb(db, "Database", "SQLite/MySQL", "存储配置和监控数据")

    Container(agent1, "Agent 1", "Go", "部署在目标服务器，执行操作")
    Container(agent2, "Agent 2", "Go", "部署在目标服务器，执行操作")
    Container(agentn, "Agent N", "Go", "部署在目标服务器，执行操作")

    Rel(user, spa, "使用", "HTTP")
    Rel(spa, api, "请求 API", "HTTP")
    Rel(api, db, "读写", "SQL")
    Rel(api, agent1, "调度/通信", "HTTP")
    Rel(api, agent2, "调度/通信", "HTTP")
    Rel(api, agentn, "调度/通信", "HTTP")
```

### 页面展示

![overview](/docs/zh-CN/images/overview.png)
...
> **前置条件**：请确保系统已安装以下命令：
> - `go` - Go 1.25+ 编译器
> - `npm` - Node.js 包管理器（Node.js 18+）
>
> 验证安装：
> ```bash
> go version    # 应显示 Go 1.25+
> npm -v        # 应显示 npm 版本号
> ```
