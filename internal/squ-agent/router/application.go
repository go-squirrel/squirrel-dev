...
func Application(group *gin.RouterGroup, conf *config.Config, db database.DB, confDB database.DB) {
	...
	service := application.New(conf, applicationRepository.New(db.GetDB()), confRepository.New(confDB.GetDB()))
	...
	group.POST("/application/start/:deployId", application.StartHandler(service))
	group.POST("/application/stop/:deployId", application.StopHandler(service))
	group.POST("/application/delete/:deployId", application.DeleteByNameHandler(service))
}
