...
import type { NetworkIORecord, TimeRange } from '@/types/monitor'
...
const getChartOption = () => {
  ...
  // 使用时间戳作为 x 轴数据
  const uploadDataPoints = speedData.value.map(d => [
    new Date(d.collect_time).getTime(),
    d.upload_speed
  ])
  const downloadDataPoints = speedData.value.map(d => [
    new Date(d.collect_time).getTime(),
    d.download_speed
  ])
...
watch([() => props.data, () => props.timeRange, selectedInterface], () => nextTick(() => updateChart()), { deep: true })
</script>
